...
<tr class="item" id="<?php echo sprintf('luccaItem_%s', $luccaOriginalItem['Item']['id']); ?>">
	...
	<td class="quantity divider"><?php echo $luccaOriginalItem['ItemNY']; ?></td>
	<td class="quantity divider"><?php echo $luccaOriginalItem['ItemLA']; ?></td>
	<td class="quantity divider"><?php echo $luccaOriginalItem['ItemWH']; ?></td>
	<td class="note-link">
		<?php echo $html->link(sprintf('products(%s)', $luccaOriginalItem['ChildrenCount']), '', array('class' => 'products-block', 'itemId' => $luccaOriginalItem['Item']['id'])); ?>&nbsp;|&nbsp;
		<?php echo $html->link(sprintf('orders(%s)', $luccaOriginalItem['OrderCount']), '', array('class' => 'orders-block')); ?>&nbsp;|&nbsp;
		<?php echo $html->link(sprintf('notes(%s)', $luccaOriginalItem['NoteCount']), '', array('class' => 'notes-block')); ?>
	</td>
</tr>
<tr>
	<td class="image">&nbsp;</td>
	...
	<?php foreach ($luccaOriginalItem['Children'] as $children): ?>
		<tr class="products-rows" parentItem="<?php echo $luccaOriginalItem['Item']['id']; ?>">
			<td class="image"></td>
			<td class="name divider"><?php echo $html->link($children['Item']['name'], array('controller' => 'item', 'action' => 'summary', 'prefix' => 'admin', $children['Item']['id'])); ?></td>
			<td class="description divider"><?php echo $children['Item']['description']; ?></td>
			<td class="quantity divider"><?php echo $children['ItemNY']['quantity']; ?></td>
			<td class="quantity divider"><?php echo $children['ItemLA']['quantity']; ?></td>
			<td class="quantity divider"><?php echo $children['ItemWH']['quantity']; ?></td>
			<td class="note-link">
				<?php echo $html->link(sprintf('notes(%s)', $children['NoteCount']), '', array('class' => 'notes-block')); ?>
			</td>
		</tr>
		<tr>
			<td class="image">&nbsp;</td>
			<td colspan="6">
				...
				<div class="notesForm">
				<form id="itemNotes" method="post" action="/admin/orders/save_note/">
				<input type="hidden" name="data[Note][item]" value="<?php echo $children['Item']['id']; ?>" />
				<textarea name="data[Note][note]">Start Typing...</textarea>
				<div class="hiddenFields">
					<div class="subheading">To:</div>
						<div>
				...
				<div class="notesList notes">
					<?php foreach ($children['Note'] as $note): ?>
						<div class="note">
							<p><?php echo date('D, j M \a\t g:ia', strtotime($note['Note']['created'])); ?></p>
							<p><?php echo $note['Note']['note']; ?></p>
							<div class="bottomMenu"><a href="#">edit</a><input type="hidden" value="<?php echo $note['Note']['id']; ?>" name="id" />&nbsp;|&nbsp;<a href="#">comments(<?php echo count($note['Comments']); ?>)</a></div>
				...
				<?php if ($children['NoteCount'] > 10): ?>
				<div class="view-all-notes"><?php echo $html->link('view all notes', array('controller' => 'item', 'action' => 'summary', 'prefix' => 'admin', $children['Item']['id'])); ?></div>
				<?php endif; ?>
				</div>
			</td>
		</tr>
