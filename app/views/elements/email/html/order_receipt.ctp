...
<h2>Hi <?php echo $person['first_name'] ?>. Thank you for placing your order with Lucca Antiques</h2>

<p>Please remember to contact us regarding when you will be picking up your items.</p>



<p>Below is a receipt of your order. Please keep this receipt for your records.</p>
	...
	<?php if(count($ordered_items) >= 1) {$i = 0 ; ?>
		...
		<?php if( isset($o['addon']) && $o['addon']['price'] !== '') { ?>
				<tr>
					...
					<td class="price">$ <?php echo number_format($o['addon']['price']) ?></td>
...
<div class="subtotal">
<p>Subtotal: $<?php echo number_format($subtotal, 2, '.', ',') ?><br/>

	<?php if(isset($sales_tax_amount) && $sales_tax_amount >0) { ?>
Sales Tax: $<?php echo number_format($sales_tax_amount, 2, '.', ',') ?></p>
